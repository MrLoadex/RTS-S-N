...
public class UIManager : Singleton<UIManager>
{
    ...
    [SerializeField] private GameObject recursoColocadoPanel;
    ...
    [Header("Recurso colocado Config")]
    [SerializeField] private TextMeshProUGUI nombreRecursoColocadoTMP;
    [SerializeField] private TextMeshProUGUI descripcionRecursoColocadoTMP;
    [SerializeField] private Image iconoRecursoColocado;
    [SerializeField] private Image barraRestanteRecursoColocadoIMG;
    [SerializeField] private TextMeshProUGUI restanteRecursoColocadoTMP;

    [Header("OleadaConfig")]
    [SerializeField] private TextMeshProUGUI oleadaActualTMP;
    ...
    public void ConfigurarOleadaUI(int _oleadaActual, int _segundosParaOleada, int _zombisVivos)
    {
        ...
        {
            if (tiempoSigOleadaTMP.color != Color.black) 
            {
        ...
        // Actualizar GUI
        oleadaActualTMP.text = "Ronda " + _oleadaActual.ToString();
        tiempoSigOleadaTMP.text = tiempoFormateado;
        zombisVivosTMP.text = _zombisVivos.ToString();
    }

    IEnumerator IniciarBordeSangriento(bool aumentarVision)
    {
        while (bordeSangrientoIMG.gameObject.activeSelf) 
        {
            yield return new WaitForSeconds(0.1f);

            Color nuevoColor = bordeSangrientoIMG.color;
            ...
            if (aumentarVision)
            {
                ...
                if (nuevoColor.a >= 1f) 
                {
                    aumentarVision = false; // Cambiar a disminuir cuando llega al máximo
                }
            }
            else 
            {
                nuevoColor.a -= 0.03f;
                if (nuevoColor.a <= 0.15f) 
                {
                    aumentarVision = true; // Cambiar a aumentar cuando llega al mínimo
                }
            }

            nuevoColor.a = Mathf.Clamp01(nuevoColor.a);
            bordeSangrientoIMG.color = nuevoColor;
        }
    }
