...
public class BuilderManager : Singleton<BuilderManager>
{
    ...
    {
        // Si no está construyendo o no hay edificio
        ...
        {
            // Obtener la ubicación del punto objetivo
            edificioOBJ.transform.position = new Vector3(hit.point.x, hit.point.y + ajusteAlturaEdificio, hit.point.z);

            // Calcular la rotación deseada en los ejes x y z
            Quaternion targetRotationXZ = Quaternion.FromToRotation(Vector3.up, hit.normal);
            
            // Mantener la rotación original en el eje y
            Quaternion originalRotationY = Quaternion.Euler(0, edificioOBJ.transform.rotation.eulerAngles.y, 0);

            // Combinar las rotaciones
            edificioOBJ.transform.rotation = originalRotationY * targetRotationXZ;

            // Verificar si la superficie golpeada es adecuada
